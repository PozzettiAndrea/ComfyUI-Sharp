...
# Heavy / optional deps. The depth-merge path in Sharp only needs
# `utils3d.np.uv_map` (used by merge_panorama_depth_gpu) and scipy/torch
# (lsmr solver). open3d / trimesh / PIL are only touched by the
# point-cloud / mesh / image helpers further down in this file — none
# of which the Sharp depth_merge node calls. Make them optional so
# Sharp's nodes package imports cleanly even when those libs aren't in
# the env.
try:
...
def merge_panorama_depth_gpu(width: int, height: int, distance_maps: List[np.ndarray], pred_masks: List[np.ndarray], extrinsics: List[np.ndarray], intrinsics: List[np.ndarray]):
    ...
    # --- One-shot upload of the per-face data + camera params. ---
    ...
    log_dist_gpu = torch.log(dist_gpu.clamp(min=1e-30))                                # (N, fh, fw)
    mask_stack = np.stack([m.astype(np.uint8, copy=False) for m in pred_masks])         # (N, fh, fw)
    pred_mask_gpu = torch.from_numpy(mask_stack).to(device).bool()
    ...
    R = ext_gpu[:, :3, :3]                          # (N, 3, 3)
    t = ext_gpu[:, :3, 3]                           # (N, 3)
    p_cam = torch.einsum('nij,pj->npi', R, dirs) + t.unsqueeze(1)  # (N, H*W, 3)
    p_proj = torch.einsum('nij,npj->npi', intr_gpu, p_cam)         # (N, H*W, 3)
    depth = p_cam[..., 2]                                          # (N, H*W) camera-Z

    # Safe divide: points behind the camera get garbage uv but the validity
    # mask catches them. Avoids NaN poisoning grid_sample below.
    safe_w = torch.where(
        p_proj[..., 2:3] > 1e-12,
        p_proj[..., 2:3], torch.ones_like(p_proj[..., 2:3]),
    )
    projected_uv = p_proj[..., :2] / safe_w                        # (N, H*W, 2) in [0, 1] when valid
    projection_valid = (
        (depth > 0)
        & (projected_uv[..., 0] >= 0) & (projected_uv[..., 0] <= 1)
        & (projected_uv[..., 1] >= 0) & (projected_uv[..., 1] <= 1)
    ).reshape(N, height, width)                                    # (N, H, W) bool

    # grid_sample convention: normalized [-1, +1] with align_corners=False
    # = half-pixel-centre (centre of pixel 0 maps to -1 + 1/W). For
    # utils3d-normalized uv (uv = (pixel + 0.5) / N), the conversion is
    # the simple `grid = 2*uv - 1`.
    grid = (projected_uv.clamp(0.0, 1.0) * 2.0 - 1.0).reshape(N, height, width, 2)

    # --- Batched bilinear sample of log-distance + nearest sample of mask. ---
    sampled_log_dist = F.grid_sample(
        log_dist_gpu.unsqueeze(1),                                 # (N, 1, fh, fw)
        grid,
        mode='bilinear', padding_mode='border', align_corners=False,
    ).squeeze(1)                                                   # (N, H, W)
    sampled_pred_mask = F.grid_sample(
        pred_mask_gpu.float().unsqueeze(1),
        grid,
        mode='nearest', padding_mode='border', align_corners=False,
    ).squeeze(1) > 0.5                                             # (N, H, W) bool

    # Zero-out log-distance where projection is invalid (matches the CPU
    # `np.where(projection_valid_mask, cv2.remap(...), 0)`). Same with the
    # final per-face mask: AND of projection-valid and resampled-pred-mask.
    panorama_log_distance = torch.where(
        projection_valid, sampled_log_dist,
        torch.zeros_like(sampled_log_dist),
    )                                                              # (N, H, W)
    panorama_pred_mask_per_face = projection_valid & sampled_pred_mask   # (N, H, W)

    # --- Batched gradient computation (wrap in x, no wrap in y). ---
    # CPU equivalent: np.pad(((0,0),(0,1)), mode='wrap'); diff. Result
    # shapes (H, W) for grad_x, (H-1, W+1) for grad_y.
    log_dist_xpad = torch.cat(
        [panorama_log_distance, panorama_log_distance[:, :, :1]], dim=-1
    )                                                              # (N, H, W+1)
    grad_x = log_dist_xpad[:, :, :-1] - log_dist_xpad[:, :, 1:]    # (N, H, W)
    grad_y = log_dist_xpad[:, :-1, :] - log_dist_xpad[:, 1:, :]    # (N, H-1, W+1)

    mask_xpad = torch.cat(
        [panorama_pred_mask_per_face, panorama_pred_mask_per_face[:, :, :1]], dim=-1
    )                                                              # (N, H, W+1)
    mask_x = mask_xpad[:, :, :-1] & mask_xpad[:, :, 1:]            # (N, H, W)
    mask_y = mask_xpad[:, :-1, :] & mask_xpad[:, 1:, :]            # (N, H-1, W+1)

    # --- Batched 5-point Laplacian + 5-neighbour validity. ---
    # CPU does np.pad y-edge, x-wrap then scipy.ndimage.convolve with the
    # symmetric Laplacian kernel. We do the same padding then direct
    # neighbour slicing — equivalent up to fp32 round-off and avoids the
    # conv2d kernel launch overhead.
    log_dist_4d = panorama_log_distance.unsqueeze(1)               # (N, 1, H, W)
    log_dist_pad = F.pad(log_dist_4d, (0, 0, 1, 1), mode='replicate')
    log_dist_pad = F.pad(log_dist_pad, (1, 1, 0, 0), mode='circular')  # (N, 1, H+2, W+2)
    center = log_dist_pad[:, 0, 1:-1, 1:-1]                        # (N, H, W)
    up = log_dist_pad[:, 0, :-2, 1:-1]
    down = log_dist_pad[:, 0, 2:, 1:-1]
    left = log_dist_pad[:, 0, 1:-1, :-2]
    right = log_dist_pad[:, 0, 1:-1, 2:]
    laplacian = up + down + left + right - 4.0 * center            # (N, H, W)

    mask_4d = panorama_pred_mask_per_face.unsqueeze(1).float()     # (N, 1, H, W)
    mask_pad = F.pad(mask_4d, (0, 0, 1, 1), mode='replicate')
    mask_pad = F.pad(mask_pad, (1, 1, 0, 0), mode='circular')      # (N, 1, H+2, W+2)
    mc = mask_pad[:, 0, 1:-1, 1:-1] > 0.5
    mu = mask_pad[:, 0, :-2, 1:-1] > 0.5
    md = mask_pad[:, 0, 2:, 1:-1] > 0.5
    ml = mask_pad[:, 0, 1:-1, :-2] > 0.5
    mr = mask_pad[:, 0, 1:-1, 2:] > 0.5
    mask_laplacian = mc & mu & md & ml & mr                        # (N, H, W) — bit-equiv to convolve(==5)

    # --- Weighted reductions across the face dimension. ---
    # CPU: np.sum(maps * mask, axis=0) / np.sum(mask, axis=0).clip(1e-3).
    # GPU equivalent — fused into 3 reductions, one each for grad_x,
    # grad_y, laplacian.
    mask_x_f = mask_x.float()
    mask_y_f = mask_y.float()
    mask_lap_f = mask_laplacian.float()
    avg_grad_x = (grad_x * mask_x_f).sum(0) / mask_x_f.sum(0).clamp(min=1e-3)         # (H, W)
    avg_grad_y = (grad_y * mask_y_f).sum(0) / mask_y_f.sum(0).clamp(min=1e-3)         # (H-1, W+1)
    avg_lap = (laplacian * mask_lap_f).sum(0) / mask_lap_f.sum(0).clamp(min=1e-3)     # (H, W)

    any_mask_x = mask_x.any(0)                                                        # (H, W)
    any_mask_y = mask_y.any(0)                                                        # (H-1, W+1)
    ...
    panorama_pred_mask_final_np = panorama_pred_mask_final.cpu().numpy()
